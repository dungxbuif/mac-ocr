	...
	return gin.H{
		...
		"paths": gin.H{
			...
			"/v1/uploads/presign": gin.H{"post": secured(operationWithBody("Create a presigned upload URL", "Uploads", refSchema("PresignUploadRequest"), gin.H{
				...
				"401": problemResponse("Invalid or missing API key", problem),
				"429": problemResponse("Aggregate retained and reserved input bytes exceed the account storage quota", problem),
...
func openAPISchemas() gin.H {
	return gin.H{
		...
		"PresignUploadResponse": gin.H{"type": "object", "required": []string{"uploadUrl", "sourceUrl", "method", "expiresAt", "reservationExpiresAt", "maxUploadBytes", "sizeBytes", "headers"}, "properties": gin.H{
			...
			"reservationExpiresAt": gin.H{"type": "string", "format": "date-time", "description": "Byte-reservation expiry. Submit sourceUrl before this time; abandoned reservations are released automatically."},
